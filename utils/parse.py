...
des_prompt_text = "Descriptions: "
des_prompt_text_no_trailing_space = des_prompt_text.rstrip()
more_prompt_text = "More Description: "
more_prompt_text_no_trailing_space = more_prompt_text.rstrip()
...
def parse_input_with_negative(text=None, no_input=False):
    ...
    text_split = text.split(des_prompt_text_no_trailing_space)
    if len(text_split) == 2:
        gen_boxes, text_rem = text_split
    elif len(text_split) == 1:
        if no_input:
            raise user_error(f"Invalid input (no descriptions): {text}")
        gen_boxes = text
        text_rem = ""
        while not text_rem:
            # Ignore the empty lines in the response
            text_rem = input("Descriptions: ").strip()
        if des_prompt_text_no_trailing_space in text_rem:
            text_rem = text_rem.split(des_prompt_text_no_trailing_space)[1]
    else:
        raise user_error(f"Invalid input (possibly multiple descriptions): {text}")

    text_split = text_rem.split(more_prompt_text_no_trailing_space)
    if len(text_split) == 2:
        des_prompt, text_rem = text_split
    elif len(text_split) == 1:
        if no_input:
            raise user_error(f"Invalid input (no descriptions): {text}")
        des_prompt = text_rem
        ...
        while not text_rem:
            # Ignore the empty lines in the response
            text_rem = input("More Description: ").strip()
        if more_prompt_text_no_trailing_space in text_rem:
            text_rem = text_rem.split(more_prompt_text_no_trailing_space)[1]
    else:
        raise user_error(f"Invalid input (possibly multiple descriptions): {text}")

    text_split = text_rem.split(bg_prompt_text_no_trailing_space)

    if len(text_split) == 2:
        more_prompt, text_rem = text_split

    elif len(text_split) == 1:
        if no_input:
            raise user_error(f"Invalid input (no background prompt): {text}")

        more_prompt = text_rem
        text_rem = ""
        while not text_rem:
            text_rem = input("Background prompt: ").strip()
    ...
    if len(text_split) == 2:
        ...
        neg_prompt = ""
        if not no_input:
            neg_prompt = input("Negative prompt: ").strip()
    ...
    des_prompt = des_prompt.strip()
    more_prompt = more_prompt.strip()
    try:
        des_prompt = ast.literal_eval(des_prompt)
        more_prompt = ast.literal_eval(more_prompt)
    except SyntaxError as e:
        # Sometimes the response is in plain text
        if "No objects" in des_prompt or des_prompt.strip() == "":
            des_prompt = []
        else:
            raise e
        if "No objects" in more_prompt or more_prompt.strip() == "":
            more_prompt = []
        else:
            raise e
...
def convert_spec(spec, height, width, include_counts=True, verbose=False):
    # Infer from spec
    prompt, gen_boxes, bg_prompt, descriptions, more_des = spec['prompt'], spec['gen_boxes'], spec['bg_prompt'], spec['descriptions'], spec['more_description']
    ...
    descriptions = sorted(descriptions, key=lambda gen_box: gen_box[0])
    for i in range(len(more_des)):
        more_des[i] = sorted(more_des[i], key=lambda gen_box: gen_box[0])
    ...
    return so_prompt_phrase_word_box_list, overall_prompt, overall_phrases_words_bboxes, descriptions, more_des
